...
function CalendarView({ requests }: { requests: TimeOffRequest[] }) {
  ...
  const totalCells = firstOffset + lastDay.getDate();
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-7 gap-px bg-border rounded-lg overflow-hidden border">
        {DAY_LABELS.map((d) => (
          <div key={d} className="bg-muted/60 text-center text-xs font-medium text-muted-foreground py-2">
            {d}
          </div>
        ))}
        {Array.from({ length: rows * 7 }, (_, i) => {
          ...
          if (!isValid) {
            return <div key={i} className="bg-background min-h-[72px]" />;
          }
          ...
          return (
            <div
              key={i}
              className={`bg-background min-h-[72px] p-1.5 relative ${isToday ? "ring-2 ring-inset ring-primary/40" : ""}`}
            >
              ...
              <div className="mt-0.5 space-y-0.5">
                {dayRequests.slice(0, 3).map((req) => (
                  <div
                    key={req.id}
                    title={`${req.employeeName ?? "Employee"} — ${req.type} (${req.status})`}
                ...
                {dayRequests.length > 3 && (
                  <div className="text-[10px] text-muted-foreground px-1">+{dayRequests.length - 3} more</div>
                )}
              </div>
            </div>
          );
        ...
        <div className="space-y-4 pt-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v: typeof newRequest.type) => setNewRequest({ ...newRequest, type: v })}
            >
        ...
        {view === "calendar" ? (
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={isAdmin ? 6 : 4} className="text-center">Loading...</TableCell>
                </TableRow>
